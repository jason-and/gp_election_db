...
con = duckdb.connect('/home/andrew/projects/cgp_election_data/chicago_elections.db')
...
def import_precinct_geojson(filepath, valid_from_year, valid_to_year=None):
    ...
    if duplicates.any():
        ...
        if zeros_check.any():
            print("CRITICAL: Found '00000' precinct ID which caused the previous error!")
    ...
    result = con.execute("SELECT COALESCE(MAX(precinct_geometry_id), 0) FROM precinct_geometries").fetchone()
    start_id = result[0] + 1
    ...
    if zeros:
        print(f"WARNING: Database still contains {len(zeros)} records with precinct_id '00000'!")
...
try:
    ...
    if constraints and 'UNIQUE' in constraints[0]:
        ...
        con.execute("CREATE TABLE IF NOT EXISTS precinct_geometries_backup AS SELECT * FROM precinct_geometries")
        ...
        backup_count = con.execute("SELECT COUNT(*) FROM precinct_geometries_backup").fetchone()[0]
        if backup_count > 0:
            con.execute("""
            INSERT INTO precinct_geometries
    ...
    total_imported += import_precinct_geojson(
        '/home/andrew/projects/cgp_election_data/precincts/2010_precincts.geojson', 
        2010, 2013
    )
    con.execute("COMMIT")
    ...
    con.execute("BEGIN TRANSACTION")
    total_imported += import_precinct_geojson(
        '/home/andrew/projects/cgp_election_data/precincts/2014_precincts.geojson', 
        2014, 2021
    )
    con.execute("COMMIT")
    ...
    con.execute("BEGIN TRANSACTION")
    total_imported += import_precinct_geojson(
        '/home/andrew/projects/cgp_election_data/precincts/2022_precincts.geojson', 
        2022, None
    )
    con.execute("COMMIT")
    ...
    con.execute("CREATE INDEX IF NOT EXISTS idx_precinct_id ON precinct_geometries(precinct_id)")
